...
class BaseModels
{
    ...
    {
        $self = new static();
        $self->query = "SELECT * FROM {$self->table_name} Order By {$self->primary_key} DESC ";
        return $self->get();
    }
    ...
    public static function where1($pr)
    {
        $self = new static();
    ...
    public static function checkLogin($email, $pass) {
        $self = new static();
        $self->query = "SELECT * FROM {$self->table_name} WHERE email = '{$email}' AND pass = '{$pass}'";
        $result = $self->get();
        return empty($result[0]) ? null : $result[0];
    }
